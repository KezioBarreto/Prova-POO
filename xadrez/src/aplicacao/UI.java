...
public class UI {
	...
	public static jogoPosicao leJogoPosicao(Scanner sc) {
		try {
			...
			throw new InputMismatchException("Erro lendo jogoPosicao. Valores validos vao de a1 a h8.");
		}
	}
	
	public static void imprimePartida(Partida partida, List<PecaXadrez> capturado) {
		imprimeTabuleiro(partida.getPecas());
		System.out.println();
		imprimePecasCapturadas(capturado);
		System.out.println();
		System.out.println("Turn : " + partida.getTurno());
	...
	private static void imprimePecasCapturadas(List<PecaXadrez> capturado) {
		List<PecaXadrez> white = capturado.stream().filter(x -> x.getCor() == Cor.WHITE).collect(Collectors.toList());
		List<PecaXadrez> black = capturado.stream().filter(x -> x.getCor() == Cor.BLACK).collect(Collectors.toList());
